...
class DataGenerator:
    ...
    def parse_pcb(self, labels_filename):
        self.images_dir = ""
        ...
        with open(self.labels_filename) as labels_file:
            for img_anno_pair in labels_file:
                img_path, anno_path = img_anno_pair.split(' ')
                img_path = img_path.strip()
                anno_path = anno_path.strip().replace('\r', '')
                test_img_path = img_path[:-4] + "_test.jpg"
                temp_img_path = img_path[:-4] + "_temp.jpg"
                ...
                with open(anno_path) as anno_file:
                    for line in anno_file:
                        ...
                        x1, y1, x2, y2, class_label = line.split(' ')
                        box.append([class_label, x1, y1, x2, y2])
                    anno_file.close()
                
                self.images.append([test_img, temp_img])
                self.labels.append(np.stack(box, axis=0))
            labels_file.close()
    ...
    def parse_csv(self,
        ...
        for i, box in enumerate(data):
            ...
            if box[0] == current_file: # If this box (i.e. this line of the CSV file) belongs to the current image file
                ...
                if random_sample: # In case we're not using the full dataset, but a random sample of it. # TSL: False
                    p = np.random.uniform(0,1)
                    if p >= (1-random_sample):
                        self.labels.append(np.stack(current_labels, axis=0))
                        self.filenames.append(os.path.join(self.images_dir, current_file))
                ...
                if i == len(data)-1: # If this is the last line of the CSV file
                    if random_sample: # In case we're not using the full dataset, but a random sample of it.
                        ...
                        self.labels.append(np.stack(current_labelscurrent_labels, axis=0))
                        self.filenames.append(os.path.join(self.images_dir, current_file))
                        self.image_ids.append(current_image_id)
    ...
    def generate_pcb(self, batch_size=32, shuffle=True, argumentation = True, label_encoder=None, random_crop=512, random_scale=False):
        ...
        while True:
            ...
            batch_test_X, batch_temp_X, batch_y = [], [], []
            ...
            if not (self.images is None):
                for i in batch_indices:
                    batch_test_X.append(self.images[i][0].copy())
                    batch_temp_X.append(self.images[i][1].copy())
            batch_y = deepcopy(self.labels[current:current+batch_size])
            current += batch_size
            
            success_batch = True
            for i in range(len(batch_test_X)):

                # if not (self.labels is None):
                #   # Convert the labels for this image to an array (in case they aren't already).
                batch_y[i] = np.array(batch_y[i]).astype(np.int32)
                    
                # Apply any image transformations we may have received.
                if argumentation:
                    test_img = batch_test_X[i]
                    temp_img = batch_temp_X[i]
                    bbox = batch_y[i]
                    
                    assert test_img.shape==temp_img.shape
                    ...
                    if True:  
                        ...
                        if angle == 270:
                            ...
                            test_img = cv2.flip(test_img, 0)
                            temp_img = np.transpose(temp_img, (1,0))
                            temp_img = cv2.flip(temp_img, 0)
                        elif angle == 180:
                            test_img = cv2.flip(test_img, -1)
                            temp_img = cv2.flip(temp_img, -1)
                    ...
                    if random_scale:  # random scale
                        zoom_x = random.uniform(0.95, 1.05)
                        zoom_y = random.uniform(0.95, 1.05)
                        ...
                        test_img = cv2.resize(test_img, (int(zoom_x*test_img.shape[0]), int(zoom_y*test_img.shape[1])), cv2.INTER_NEAREST)
						
                    if random_crop > 0:
                        crop_pad = 5
                        rows, cols = test_img.shape[0:2]
                        
                        valid_rows = np.ones(rows)
                        valid_cols = np.ones(cols)
                        
                        if random_scale:
						    if rows > temp_img.shape[0]:
						        for r in range(temp_img.shape[0], rows):
						            valid_rows[r] = 0
							
						    if cols > temp_img.shape[1]:
						        for c in range(temp_img.shape[1], cols):
						            valid_cols[c] = 0
                        ...
                        for bbox_idx in range(bbox.shape[0]):
                            x1 = max(bbox[bbox_idx, 1]-crop_pad, 0)
                            x2 = min(bbox[bbox_idx, 3]+crop_pad, cols-1)
                            y1 = max(bbox[bbox_idx, 2]-crop_pad, 0)
                            y2 = min(bbox[bbox_idx, 4]+crop_pad, rows-1)
                            
                            valid_rows[y1:(y2+1)] = 0
                            valid_cols[x1:(x2+1)] = 0
                        
                        crop_rows = []
                        crop_cols = []
                        
                        for row in range(rows - random_crop):
                            if valid_rows[row] and valid_rows[row+random_crop] and row<max_y and (row+random_crop) > min_y:
                                crop_rows.append(row)
                        
                        for col in range(cols - random_crop):
                            if valid_cols[col] and valid_cols[col+random_crop] and col<max_x and (col+random_crop) > min_x:
                                crop_cols.append(col)
                                
                        if len(crop_rows)>0 and len(crop_cols)>0:
                            start_row = np.random.choice(crop_rows, 1)[0]
                            start_col = np.random.choice(crop_cols, 1)[0]
                        
                            test_img = test_img[start_row:start_row+random_crop, start_col:start_col+random_crop]
                            temp_img = temp_img[start_row:start_row+random_crop, start_col:start_col+random_crop]
                            ...
                            bbox = np.delete(bbox, np.where(bbox[:,1]<0), axis=0)
                            bbox = np.delete(bbox, np.where(bbox[:,3]<0), axis=0)
                        else:
                            success_batch = False
                            break
                    
                batch_test_X[i] = test_img
                batch_temp_X[i] = temp_img
                batch_y[i] = bbox
            ...
            if False:  # visulized
                for idx in range(batch_size):
                    test_img = batch_test_X[idx].copy()
                    test_img = cv2.cvtColor(test_img, cv2.COLOR_GRAY2BGR)
                    for bbox_id in range(batch_y[idx].shape[0]):
                        test_img = cv2.rectangle(test_img, (batch_y[idx][bbox_id, 1], batch_y[idx][bbox_id, 2]), (batch_y[idx][bbox_id, 3], batch_y[idx][bbox_id, 4]), (0, 0, 255), 2)
                    cv2.imwrite('./output_{}.jpg'.format(idx), test_img)
            batch_y_encoded = label_encoder(batch_y, diagnostics=False)
            batch_test_X = np.array(batch_test_X)
            batch_temp_X = np.array(batch_temp_X)
			
            if len(batch_test_X.shape) == 3:
                batch_test_X = np.expand_dims(batch_test_X, 3)
                batch_temp_X = np.expand_dims(batch_temp_X, 3)
            
            yield [[batch_test_X, batch_temp_X], batch_y_encoded]
