...
from keras.layers import Input, Lambda, Conv2D, MaxPooling2D, BatchNormalization, ELU, Reshape, Concatenate, Activation, Subtract, AveragePooling2D
...
def build_model(image_size,
    ...
    conv1 = Conv2D(32, (5, 5), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv1')(x1)
    # conv1 = BatchNormalization(axis=3, momentum=0.99, name='bn1')(conv1) # Tensorflow uses filter format [filter_height, filter_width, in_channels, out_channels], hence axis = 3
    # conv1 = ELU(name='elu1')(conv1)
	# conv1 = Activation('relu')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2), name='pool1')(conv1)

    conv2 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv2')(pool1)
    # conv2 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv2_2')(conv2)
    # conv2 = BatchNormalization(axis=3, momentum=0.99, name='bn2')(conv2)
    # conv2 = ELU(name='elu2')(conv2)
	# conv2 = Activation('relu')(conv2)
    pool2 = MaxPooling2D(pool_size=(2, 2), name='pool2')(conv2)

    conv3 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv3')(pool2)
    conv3 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv3_2')(conv3)
    conv3 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv3_3')(conv3)
    # conv3 = BatchNormalization(axis=3, momentum=0.99, name='bn3')(conv3)
    # conv3 = ELU(name='elu3')(conv3)
	# conv3 = Activation('relu')(conv3)
    # pool3 = MaxPooling2D(pool_size=(2, 2), name='pool3')(conv3)

    # conv4 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv4')(pool3)
    # conv4 = Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(l2_reg), name='conv4_2')(conv4)
    ...
    base_model = Model(x, conv3, name='base_model')
    ...
    test_feat = base_model(x_test) 
    temp_feat = base_model(x_temp)
    feat_shape = (img_height // 4, img_width // 4)
    pool_sizes = [1, 2, 4, 6, 12, 18, 24]
    group_size = 3
    # assert len(pool_sizes) - 3 + 1 == 3
	
    shared_feat = Subtract()([test_feat, temp_feat])
    shared_feat = Activation('tanh')(shared_feat)
	
    feat_pools = []
    feat_group = []
    for ps in pool_sizes:
        if ps == 1:
            feat_pools.append(shared_feat)
            continue
        feat_tmp = MaxPooling2D(pool_size=(ps, ps), strides=(ps, ps), padding='same')(shared_feat)
        feat_tmp = Conv2D(16, (1, 1), strides=(1, 1), use_bias=False)(feat_tmp)
        feat_tmp = BatchNormalization()(feat_tmp)
        feat_tmp = Activation('relu')(feat_tmp)
		
        feat_pools.append(feat_tmp)
	
	# 1/ 4
    target_shape = feat_shape
    feat_pyramid_layer1 = Concatenate()([feat_pools[0],
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[1]),
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[2])])
	# 1 / 16
    target_shape = K.int_shape(feat_pools[2])[1:3]
    
    feat_pyramid_layer2 = Concatenate()([feat_pools[2],
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[3]),
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[4])])
	# 1 / 48									 
    target_shape = K.int_shape(feat_pools[4])[1:3]
    
    feat_pyramid_layer3 = Concatenate()([feat_pools[4],
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[5]),
										 Lambda(Interp, arguments={'shape': target_shape})(feat_pools[6])])
	
    shared_features = [feat_pyramid_layer1, feat_pyramid_layer2, feat_pyramid_layer3]	
